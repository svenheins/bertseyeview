...
def get_meta_data(pubmed_ids_all_batches: list, bioconcepts: str = "none", 
                ...
                if 'result' in r_meta.json():
                    for pubmed_id in pubmed_ids:
                        if pubmed_id in r_meta.json()['result']:
                            ...
                            journal_list.append(get_field_or_default_value(
                                r_meta.json()['result'][pubmed_id], 
                                'fulljournalname', default="NA"))
                            pmc_id = "NA"
                            for article_id in r_meta.json()['result'][pubmed_id]['articleids']:
                                if article_id['idtype'] == 'pmc':
                                    pmc_id = article_id['value']

                                    pubtator_url_pmc = 'https://www.ncbi.nlm.nih.gov/research/pubtator-api/publications/export/biocxml?pmcids='+str(pmc_id)
                                    successful_request = False
                                    count_requests = 0
                                    while (successful_request != True):
                                        pubtator_response = request_with_delay(pubtator_url_pmc)
                                        count_requests += 1
                                        if count_requests > 1: 
                                            logging.info("count_requests = "+str(count_requests))
                                        if (pubtator_response != None):
                                            ...
                                            if len(ner_dict) > 0:
                                                ...
                                                annotations_list.append(ner_string)
    ...
    df_content = {'title': title_list, 'abstract': abstract_list, 
                  'annotations': annotations_list, 'sortpubdate': sortpubdate_list, 
                  'epubdate': epubdate_list, 'authors': authors_list, 'journal':journal_list,
                  'pmc_id': pmc_id_list
                  }
    ...
    if (len(df_content['title']) == len(pubmed_ids_all_batches)):
        ...
        logging.info("Error: index has not the same size as the metadata-dataframe -> skipping the following batch"+ str(pubmed_ids_all_batches))
    return return_df


def main():
    pubmed_ids_all_batches = [36259984,35985548,36261021,35777586,35820507,35779659,36258805,36260611,36253682,35605742,35985541,36261001,36260098,36257874,27207975,27653566]
    result = get_meta_data(pubmed_ids_all_batches, bioconcepts = "gene,disease,chemical,species,mutation,cellline")
